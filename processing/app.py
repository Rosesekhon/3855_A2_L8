...
def populate_stats():
    ...
    rows = requests.get('http://localhost:8090/buy?timestamp={last_updated}')
    rows = rows.json()
    
    print(rows)
    object = rows[0]
    for row in rows:
        if row['item_price'] > object['item_price']:
            object = row
    ...
    sell_rw = requests.get('http://localhost:8090/sell?timestamp={last_updated}')
    sell_rw = sell_rw.json()

    sell_obj = sell_rw[0]
    for row in sell_rw:
        if row['item_price'] > sell_obj['item_price']:
            sell_obj = row
